...
class TodoistNotionSyncer:
    ...
    async def sync_todoist_to_notion(self):
        await self.sync_deleted_todoist_tasks_notion()
        try:
            ...
            for task in TodoistWrapper.get_tasks():
                ...
                if await task.isNewTask():
                    logging.info(f'New task "{task.title} {task.id}"')
                    notion_id = (
                        await NotionWrapper.NotionWrapper.create_subpage_in_database(
                            task.title,
                    ...
                    relation_id = await cache.add_to_task_cache(
                        notion_id,
                        task.id,
                        NotionWrapper.notion_default_status,
                        task.iscomplete,
                    ...
                    if task.iscomplete:
                        ...
                        await notion_task.update_select_tag_on_page(
                            notion_id, NotionWrapper.notion_done_status
                        )
            await self.sync_todoist_completed_tasks_notion()
        except Exception as error:
            logging.critical(f"Error processing task #{task.id}: {error}")
            self.fail += 1
        self.sync_stats()

    async def sync_todoist_completed_tasks_notion(self):
        """
        Syncs completed tasks from todoist to notion.
        """
        logging.info("Syncing completed tasks...")
        results = await cache.query_all_noncompleted_todoist_rows()
        for result in results:
            try:
                task = TodoistWrapper.api.get_task(result[0])
                if task.is_completed:
                    notion_id = await cache.get_notion_task_from_todoist(task.id)
                    if notion_id == "":
                        continue
                    note = NotionTask(notion_id)
                    await note.update_select_tag_on_page(
                        NotionWrapper.notion_done_status
                    )
                    await cache.update_status_from_todoist(
    ...
    async def sync_deleted_todoist_tasks_notion(self):
        """
        Syncs deleted tasks from todoist to notion.
        """
        logging.info("Syncing deleted tasks...")
        rows = await cache.query_all_rows()
        for row in rows:
            try:
                ...
                await cache.delete_relation_row(row[1])
                await cache.delete_notion_task(row[2])
                await cache.delete_todoist_task(row[1])
                self.deleted += 1
            except Exception as e:
